...
function getpage(&$m,$where,$pagesize=10){
    ...
    $p->setConfig('header','<ul><li class="rows">共<b>%TOTAL_ROW%</b>条记录&nbsp;&nbsp;每页<b>'.$pagesize.'</b>条&nbsp;&nbsp;第<b>%NOW_PAGE%</b>页/共<b>%TOTAL_PAGE%</b>页</li></ul>');
    $p->setConfig('header','%NOW_PAGE%/%TOTAL_PAGE%');
...
function getpages(&$m,$where,$pagesize=10,$sql){
    $m1=clone $m;//浅复制一个模型
    if(!empty($sql)){
        $count = $m -> join('yuyue_inpatient as i on a.id=i.pid') ->where($where.$sql)->count();//连惯操作后会对join等操作进行重置
    }else{
        $count = $m ->where($where)->count();//连惯操作后会对join等操作进行重置
    }
    // echo $count;die;
...
function get_day( $date ,$rtype = '1')  
{
    ...
    if ($rtype == '2') {
      for ($i = 1; $i <= $text ; $i ++ ) {
        $da=$year."-".$month."-".$i;
        $r[$i-1]['t'] = $i;
        $r[$i-1]['w'] = date("w",strtotime($da));
        // $r[$i-1]['sw'] = 0;
        // $r[$i-1]['xw'] = 0;
        // //上午出诊
        // for ($j=0; $j <count($sw) ; $j++) { 
        //   if($i==$sw[$j]){
        //       $r[$i-1]['sw'] = 1;
        //   }          
        // }
...
/**
 * [getAllMondayOf 以星期获取本月所有日期]
 * @param  string $year_month [description]
 * @return [type]             [description]
 */
function getAllMondayOf($rq=0,$year_month = ''){
    if(empty($year_month)){
        $year_month = date("Y-m");
    }
    ...
    $mondays = array();
    for($i=1; $i<=$maxDay; $i++){
        if(date('w', strtotime($year_month."-".$i)) == $rq){
            $mondays[] = $i;
        }
    }
// echo "<pre/>";
...
function getDateFromRange($startdate, $enddate){
    ...
    for($i=0; $i<$days; $i++){
        $date[]['time'] = date('Y-m-d', $stimestamp+(86400*$i));
    }
...
function getDateRange($startdate, $enddate) {
    ...
    $datearr = [];
    while ($stime <= $etime) {
        $datearr[] = date('j', $stime);//得到dataarr的日期数组。
        $stime = $stime + 86400;
    }
    return $datearr;
}